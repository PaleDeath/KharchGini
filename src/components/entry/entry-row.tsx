...
export function EntryRow({
  ...
  const inner = (
    <>
      ...
      {isTransfer ? (
        <span
          className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-line/60 bg-raised text-muted"
          aria-hidden
        >
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className="mt-0.5 block truncate text-xs text-muted font-normal">{context}</span>
      </span>
      ...
      <div className="shrink-0 text-right">
        ...
        >
          ...
          <Money value={entry.amount} tone="plain" />
        </span>
      </div>
    </>
  );
  ...
  return (
    <button
      ...
      className={cn(shared, 'hover:bg-raised/50 active:bg-raised/70')}
    >
      {inner}
    </button>
  );
